...
(function () {
  ...
  function startViz() {
    ...
    const draw = () => {
      ...
      g.clearRect(0, 0, w, h);
      const an = window.Player.analyser;
      if (an && window.Player.current() && window.Player.current().id === track.id && !window.Player.audio.paused) {
        ...
        for (let i = 0; i < peaks.length; i++) {
          ...
          g.fillRect((i * w) / peaks.length, h - bh, w / peaks.length - 1, bh);
        }
      }
    };
