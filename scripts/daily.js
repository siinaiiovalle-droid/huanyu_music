 ...
 *
 *   npm run daily                       # 默认 3 首纯音乐 + 3 首歌曲，完成后同步到 GitHub Pages
...
function buildTrack(kind, idx, dateStr, bitrate, ordinal, existingTitles = []) {
  ...
  const rend = engine.render(plan, { outFile });
  ...
  const meta = {
    ...
    comment: kind === 'instrumental' ? '由寰宇音乐台 AI 作曲引擎实时合成的纯音乐' : '寰宇音乐台 AI 作曲引擎创作的原创中文歌曲',
  ...
  const descZh = isSong
    ? `《${title.zh}》是一首${plan.style.genre.zh}作品，${plan.bpm} BPM，${plan.keyName}。人声由语音合成引擎逐字生成，再按旋律定音高、按谱面对齐节奏，中文歌词清晰可辨。伴奏与人声全部由代码实时合成，没有一个采样来自别处。`
    : `《${title.zh}》是一首${plan.style.genre.zh}纯音乐，${plan.bpm} BPM，${plan.keyName}。全曲由寰宇音乐台 AI 作曲引擎实时合成：和声、旋律、音色与混音都由算法在发布当天现场生成，没有一个采样来自别处。`;
  const descEn = isSong
    ? `"${title.en}" is a ${plan.style.genre.en} track at ${plan.bpm} BPM in ${plan.keyName}. The vocal is generated syllable by syllable, tuned to the melody and aligned to the score, keeping the Chinese lyrics clearly intelligible. Everything is synthesized in real time from code — no borrowed samples.`
  ...
  const track = {
    id,
    kind: isSong ? 'song' : 'instrumental',
    title: mk(title.zh, title.en),
    subtitle: isSong ? mk('原创歌曲 · 中文演唱', 'Original song · Sung in Chinese') : mk(`纯音乐 · ${plan.style.genre.zh}`, `Instrumental · ${plan.style.genre.en}`),
    description: mk(descZh, descEn),
    credits: isSong
      ? mk('作词 / 作曲 / 编曲：寰宇音乐台 AI 作曲引擎　演唱：中文语音合成声库', 'Lyrics, composition and arrangement by the Huanyu Music AI engine; vocal by the Chinese speech synthesis voice')
      : mk('作曲 / 编曲 / 混音：寰宇音乐台 AI 作曲引擎', 'Composed, arranged and mixed by the Huanyu Music AI engine'),
    cover: coverRel,
    audio: audioRel,
    ...
    peaks: rend.peaks,
    lyrics: isSong ? { lang: 'zh', languages: ['zh', 'en'], lines: engine.lyricTimeline(plan, seq) } : null,
    plays: 0,
    likes: 0,
    featured: false,
    releasedAt: today(),
...
function main() {
  ...
  const nSong = Number(args.songs ?? 3);
